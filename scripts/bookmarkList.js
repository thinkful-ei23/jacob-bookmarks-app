...
const bookmarkList = (function() {
  ...
  const expandBookmarks = function() {
    $('.js-bookmark-list').on('click', '.js-li-expand', function(event) {
      ...
      if (bookmarkObject.expanded === true) {
        $('.js-star-rating').append(`<p>${item.desc}</p><a href="${item.url}">Visit ${item.title}</a><button class="js-delete-bookmark">Delete</button>`);
      }
    });
  };
